%WATERSHEDMAXIMA   Detect significant local minima
%
% WATERSHEDMAXIMA returns a binary image with pixels set that belong to local
% maxima of the input.
%
% SYNOPSIS:
%  image_out = watershedmaxima(image_in,connectivity,max_depth,max_size,flag)
%
% PARAMETERS:
...
%
% NOTES:
%  As opposed to the function MAXIMA, local maxina are defined here as the
%  highest-valued pixels in each watershed basin, after merging the less
%  significant basins. This removes maxina that are not significant.
%
%  There can be multiple disjoint groups of pixels having the same highest
...
function out = watershedmaxima(varargin)
out = dip_morphology('watershedmaxima',varargin{:});
