...
void init_statistics( py::module& m ) {
   m.def( "Count", py::overload_cast< dip::Image const&, dip::Image const& >( &dip::Count ),
         "in"_a,
         "mask"_a = dip::Image{} );
   // UnsignedArray MaximumPixel( Image const& in, Image const& mask = {}, String const& positionFlag = "first" );
   // UnsignedArray MinimumPixel( Image const& in, Image const& mask = {}, String const& positionFlag = "first" );
   ...
   m.def( "SampleStatistics", []( dip::Image const& in, dip::Image const& mask ) {
          ...
          "in"_a,
          "mask"_a = dip::Image{} );
   // FloatArray CenterOfMass( Image const& in, Image const& mask = {} );
   // MomentAccumulator Moments( Image const& in, Image const& mask = {} );
