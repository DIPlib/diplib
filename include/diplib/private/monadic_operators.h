...
DIP__MONADIC_OPERATOR_FLOAT( Round, []( auto its ) { return std::round( *its[ 0 ] ); }, DataType::Class_Float )

/// \brief Computes the smallest integer larger or equal to each sample (rounds up).
/// Only defined for floating-point types, the output is the same type.
DIP__MONADIC_OPERATOR_FLOAT( Ceil, []( auto its ) { return std::ceil( *its[ 0 ] ); }, DataType::Class_Float )

/// \brief Computes the largest integer smaller or equal to each sample (rounds down).
/// Only defined for floating-point types, the output is the same type.
DIP__MONADIC_OPERATOR_FLOAT( Floor, []( auto its ) { return std::floor( *its[ 0 ] ); }, DataType::Class_Float )

/// \brief Computes the truncated value of each sample (rounds towards zero).
/// Only defined for floating-point types, the output is the same type.
DIP__MONADIC_OPERATOR_FLOAT( Truncate, []( auto its ) { return std::trunc( *its[ 0 ] ); }, DataType::Class_Float )
...
DIP__MONADIC_OPERATOR_FLEX( Cos, []( auto its ) { return std::cos( *its[ 0 ] ); }, DataType::Class_NonBinary )

/// \brief Computes the tangent of each sample.
DIP__MONADIC_OPERATOR_FLEX( Tan, []( auto its ) { return std::tan( *its[ 0 ] ); }, DataType::Class_NonBinary )

/// \brief Computes the arc sine of each sample.
DIP__MONADIC_OPERATOR_FLOAT( Asin, []( auto its ) { return std::asin( *its[ 0 ] ); }, DataType::Class_Real )

/// \brief Computes the arc cosine of each sample.
DIP__MONADIC_OPERATOR_FLOAT( Acos, []( auto its ) { return std::acos( *its[ 0 ] ); }, DataType::Class_Real )

/// \brief Computes the arc tangent of each sample.
DIP__MONADIC_OPERATOR_FLOAT( Atan, []( auto its ) { return std::atan( *its[ 0 ] ); }, DataType::Class_Real )

/// \brief Computes the hyperbolic sine of each sample.
DIP__MONADIC_OPERATOR_FLOAT( Sinh, []( auto its ) { return std::sinh( *its[ 0 ] ); }, DataType::Class_Real )

/// \brief Computes the hyperbolic cosine of each sample.
DIP__MONADIC_OPERATOR_FLOAT( Cosh, []( auto its ) { return std::cosh( *its[ 0 ] ); }, DataType::Class_Real )
...
DIP__MONADIC_OPERATOR_FLOAT( BesselJ1, []( auto its ) { return static_cast< decltype( *its[ 0 ] ) >( BesselJ1( *its[ 0 ] )); }, DataType::Class_Real )

/// \brief Computes the Bessel functions of the first kind of each sample, of order `alpha`.
DIP__MONADIC_OPERATOR_FLOAT_WITH_PARAM( BesselJN, dip::uint, alpha, [ alpha ]( auto its ) { return static_cast< decltype( *its[ 0 ] ) >( BesselJN( *its[ 0 ], alpha )); }, DataType::Class_Real )

/// \brief Computes the Bessel functions of the second kind of each sample, of order alpha = 0.
DIP__MONADIC_OPERATOR_FLOAT( BesselY0, []( auto its ) { return static_cast< decltype( *its[ 0 ] ) >( BesselY0( *its[ 0 ] )); }, DataType::Class_Real )

/// \brief Computes the Bessel functions of the second kind of each sample, of order alpha = 1.
DIP__MONADIC_OPERATOR_FLOAT( BesselY1, []( auto its ) { return static_cast< decltype( *its[ 0 ] ) >( BesselY1( *its[ 0 ] )); }, DataType::Class_Real )
