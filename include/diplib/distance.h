...
///     \ref dip::VectorDistanceTransform.
///
...
///
/// !!! literature
///     - R. van den Boomgaard, "Mathematical Morphology--Extensions towards Computer Vision",
...
///     - Q.Z. Ye, "The signed Euclidean distance transform and its applications",
///       in: 9^th^ International Conference on Pattern Recognition, 495-499, 1988.
...
/// The norm of `out` is identical to the result of \ref dip::EuclideanDistanceTransform.
///
/// See \ref dip::EuclideanDistanceTransform for detailed information about the parameters. Valid `method` strings are
/// `"fast"`, `"ties"`, `"true"` and `"brute force"`. That is, `"separable"` or `"square"` are not allowed.
///
/// `in` should not have any dimension larger than 10^7^ pixels, otherwise the vector components will underflow.
...
/// must be scalar. `out` will have type \ref dip::DT_SFLOAT. If `grey` is not forged, it is assumed to be valued 1
/// everywhere.
///
...
/// `out` will have type \ref dip::DT_SFLOAT.
///
/// This function is currently implemented in terms of \ref dip::GreyWeightedDistanceTransform, see that function for
