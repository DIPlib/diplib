...
/// \brief Holds 1D probability density functions and other types of distribution.
///
/// This is a container class to hold results of certain type of analysis that compute
/// a property as a function of scale, distance or intensity.
///
...
/// *N*&times;*M*, where *M* is 1 for a vector-like set of values.
///
/// Indexing into the object yields a \ref dip::Distribution::Sample, which references the
/// data in the distribution and can be used to modify it. For example, one can do
/// ```cpp
/// dip::Distribution distribution(10);
/// distribution[0].X() = 1;
/// distribution[0].Y() += 0.25;
/// ```
/// However, the \ref X and \ref Y functions of the `dip::Distribution` object itself always
/// return copies of the data, and will not allow modification of the object. One can also
/// use iterators to access all samples in turn.
///
/// Elements can be modified in such a way that *x* is no longer sorted. This could cause
/// some functions such as \ref Integrate to produce wrong results. The `Sort` method applies
/// a stable sort to restore the order.
///
/// !!! par
///     There are strong similarities between density functions and histograms. We chose
///     to keep \ref dip::Histogram and `dip::Distribution` as separate entities because their
///     use cases are very distinct, which means they require different properties and are used
///     as inputs to different types of functions. The main differences between these two classes
///     is that a histogram maps a multi-valued index to a single count ($\mathbb{D}^n \to \mathbb{N}$),
///     whereas a distribution maps a single index to multiple values ($\mathbb{D} \to \mathbb{R}^n$).
///     For both, the input domain $\mathbb{D} \subset \mathbb{R}$, and can be arbitrarily sampled.
///
///     A `dip::Histogram` with a 1D histogram can be cast to a `dip::Distribution`, but the reverse
///     is not possible.
class DIP_NO_EXPORT Distribution {
   public:
      ...
      /// \brief One immutable sample of a distribution, see \ref Sample for details.
      ...
      /// the original distribution up to that element. The *x* values must be sorted (see `Sort`).
      ...
      /// the original distribution at that element. The *x* values must be sorted (see `Sort`).
      ...
      /// \brief Fills the values for the *x* axis.
      ///
      /// If the `pixelSize` is isotropic and has physical units, the first *x* value will be
      /// `pixelSize[0].magnitude * offset`, and subsequent values will have linear increments of
      /// `pixelSize[0].magnitude * scaling`. Otherwise, `pixelSize` is ignored.
      ///
