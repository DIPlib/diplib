/*
 * DIPlib 3.0
 * This file defines the "ExcessKurtosis" measurement feature
 *
...
class FeatureExcessKurtosis : public LineBased {
   public:
      FeatureExcessKurtosis() : LineBased( { "ExcessKurtosis", "Excess Kurtosis (gamma_2) of object intensity", true } ) {};
      ...
      virtual ValueInformationArray Initialize( Image const& label, Image const& grey, dip::uint nObjects ) override {
         ...
         out[ 0 ].name = String( "ExcessKurtosis" );
         return out;
      }
      ...
      virtual void Finish( dip::uint objectIndex, Measurement::ValueIterator output ) override {
         ...
         if( data.number > 1 ) {
            ...
            dfloat h4 = data.sum4 / data.number;
            dfloat m2 = h2 - h1 * h1;
            dfloat m4 = h4 - 4 * h1 * h3 + 6 * h1 * h1 * h2 - 3 * h1 * h1 * h1 * h1;
            *output = ( m2 != 0.0 ) ? ( m4 / ( m2 * m2 ) - 3.0 ) : ( 0.0 );
         } else {
            *output = 0;
         }
      }
