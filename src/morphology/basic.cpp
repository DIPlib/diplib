...
DOCTEST_TEST_CASE("[DIPlib] testing the basic morphological filters") {
   ...
   DOCTEST_CHECK(( dip::uint )out.At( 64, 35 )[ 0 ] == pval ); // Is that pixel in the right place?
   ...
   DOCTEST_CHECK(( dip::uint )out.At( 64, 35 )[ 0 ] == pval ); // Is the origin in the right place?
   ...
   dip::BasicMorphology( out, out, se, {}, false /*compute erosion*/, true /* mirror */ );
   DOCTEST_CHECK( dip::Count( out ) == 1 ); // Did the erosion return the image to a single pixel?
   DOCTEST_CHECK(( dip::uint )out.At( 64, 35 )[ 0 ] == pval ); // Is the main pixel in the right place and with the right value?
}
