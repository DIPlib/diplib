...
DOCTEST_TEST_CASE("[DIPlib] testing the finite difference filter") {
   ...
   DOCTEST_CHECK( static_cast< dip::dfloat >( out.At( 128 )) == 0.5 );
   ...
   DOCTEST_CHECK( static_cast< dip::dfloat >( out.At( 128 )) == doctest::Approx( 1.0 ));
   img = img * img;
   out = dip::FiniteDifference( img, { 2 } );
   DOCTEST_CHECK( static_cast< dip::dfloat >( out.At( 128 )) == doctest::Approx( 2.0 ));
}
