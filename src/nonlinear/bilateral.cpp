...
public:
   ...
   virtual void SetNumberOfThreads( dip::uint threads, PixelTableOffsets const& pixelTable ) override {
   ...
   virtual void Filter( Framework::FullLineFilterParameters const& params ) override {
      ...
      for( dip::uint ii = 0; ii < length; ++ii ) {
         ...
         for( auto pto = pixelTableOffsets.begin(); pto != pixelTableOffsets.end(); ++pto ) {
            ...
            norm += weight;
            ++w;
            //*out += static_cast<FloatType<TPI>>(*w++) * in[ pto.Offset() ];
         }
         *out = sum / norm;
         in += inStride;
         out += outStride;
      ...
      /* From separable bilateral filter:
      auto filterSrc = static_cast< dfloat const* >(filter.data());
      dip::uint filterSize = filter.size();
      auto filterEnd = filterSrc + filterSize;
      ...
      for( dip::uint ii = 0; ii < length; ++ii ) {
         ...
         TPI const tonalCenter = *est;
         // Iterate over filter
         for( auto f = filterSrc; f != filterEnd; ++f, ++in_t ) {
            dip::uint luIndex = static_cast<dip::uint>( std::min( static_cast<dip::uint>( abs<TPI>( (*in_t) - tonalCenter )*tonalGaussScaling_), tonalGaussSize ) );
...
void BilateralFilter(
   ...
   DIP_START_STACK_TRACE
      ...
      // TODO: remove NoMultiThreading option after debugging is done
      Framework::Full( in, out, bufferDataType, bufferDataType, outputDataType, in.TensorElements(), { BoundaryCondition::DEFAULT }, kernel, *lineFilter, Framework::FullOption::AsScalarImage );// +Framework::FullOption::NoMultiThreading );
   DIP_END_STACK_TRACE
...
void QuantizedBilateralFilter(
   ...
   for( dip::uint i = 0; i < tonalBins.size(); ++i ) {
      ...
      // Compute lutImages[ i ]
      NormalizedConvolution( in, tonalWeight, lutImages[ i ], spatialSigmas, S::BEST, { S::DEFAULT }, truncation );
   }
...
void SelectBins( Histogram const& hist, UnsignedArray& binIndices, dip::uint first, dip::uint last, dip::uint minDist, dfloat minSample ) {
   Image subHist = hist.GetImage().At( Range( first, last ) );
   if( (first >= last) || (Sum( subHist ).As< dip::uint >() < minSample) ) {
      return;
   } else {
...
FloatArray SelectChannels(
   ...
   dip::uint minDist = static_cast<dip::uint>(floor( minDistPC * hist.Bins() / 100.0 ));
   dfloat minSample = minSamplePC * hist.Count() / 100.0;

   // Select bins
   UnsignedArray binIndices;
   binIndices.push_back( 0 );
   SelectBins( hist, binIndices, minDist, hist.Bins() - 1 - minDist, minDist, minSample);
   binIndices.push_back( hist.Bins() - 2 );  // TODO: why the forelast one and not the last one?
...
void SeparableBilateralFilter(
   ...
   DIP_START_STACK_TRACE
      ...
      // TODO: remove NoMultiThreading option after debugging is done
      Framework::Separable( in, out, bufferDataType, outputDataType, process, borders, { BoundaryCondition::DEFAULT }, *lineFilter, Framework::SeparableOption::AsScalarImage );// +Framework::SeparableOption::NoMultiThreading);
   DIP_END_STACK_TRACE
}
...
void Bilateral(
   ...
   if( method == "full" ) {
      ...
      FloatArray tonalBins = SelectChannels( in ); // TODO: this gives slightly different bin centers than bilateralf.m
      QuantizedBilateralFilter( in, estimate, out, spatialSigmas, tonalSigma, tonalBins, truncation );
   }
   else if( method == "xysep" ) {
      SeparableBilateralFilter( in, estimate, out, {}, spatialSigmas, tonalSigma, truncation );
   } else {
      DIP_THROW( "Unknown bilateral filtering method: " + method );
   }
}
